...
It's also very importnt to note, that these VMs are **only accessible from a University of Adelaide campus**, or when connecting using the **University of Adelaide VPN**.
...
Before the release of RStudio in 2012, most people would write R Scripts, and these are still a common format.
We advocate using RMarkdown as a superior alternative to R Scripts, but let's have a quick look at an R Script first.
...
Alternatively, you may like to consider that for many analyses, you'll have to return to them after a considerable break, and you also won't have access to your brain as it was while you performed the analysis.
...
**Hot Tip: No, you won't always remember why you did something! Don't kid yourself. Why on earth would you want to make your life harder by not commenting your code?**  
...
There is no magic to these, but they do help us develop good practice.
...
In the top right of your RStudio screen, you'll see the following symbol: ![](images/rproject_arrow.png).
...
If we'd wanted to include the output from `max(x)`, we would've had to copy the output from the Console, and place it in our script as a comment.
...
The R Markdown format, simply allows ambedding of R code alongside our clearly formatted, paragraphs and sections.

As we'll see, when we create an R Markdown document we have our code in one document, which we then compile into a PDF, HTML report, `ioslides` or even an MS Word document or PowerPoint presentation.
...
Notice there are a few different colours here indicating sections and R code.
We'll explore this in detail next, but in essence this is all just plain text. 
