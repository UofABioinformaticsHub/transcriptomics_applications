...
Click the staggered text symbol (![](images/staggeredText.png)) at the top of your R Markdown page.

Here you'll see all of the section headings and clicking on these will take you to that part of the document.
Chunk names can also be shown, but we'll need to set this manually.
...
In reality, we'll start with an empty document and will build up a workflow from scratch.
To do this, we'll probably need to write code and check that it works as we go.

As we may have ascertained, code within an R Markdown document has not been executed, but is just a plain text record of some code.
...
Some people like to use this setting as the default, but many others prefer to execute code in the Console without this preview.
This tends to keep a document looking cleaner while you work on it, as the previews can take up a fair bit of space.
...
This method can be used for executing a single line, but to execute every line in a chunk, you can use the <kbd>Ctrl</kbd> + <kbd>Alt</kbd> + C shortcut.
...
The 'down arrow' can be used to run all chunks above the current one.

## Writing our own code

As a final step for this week's practical, let's create our own content.
Using your mouse select everything from the `## R Markdown` header to the end of the file and delete.
...
Make a section header: `Creating an R Object`, and enter text similar to the below after this header
...
Or we can use the shortcut <kbd>Ctrl</kbd> + <kbd>Alt</kbd> + I to create an empty chunk.
...
Include the code to create `x` with the values `1:5` and **compile the document to make sure you haven't made any mistakes.**
...
> We can also perform mathematical operations directly on our entire vector in a single command. This is much simpler than selecting a range of values in Excel, and printing the results to another cell in an Excel workbook.
