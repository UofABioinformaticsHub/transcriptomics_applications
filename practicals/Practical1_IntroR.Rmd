...
Click on this and type `transcriptomic_applications`, ensuring everything is lower case and you use an underscore between words, **NOT** white-space.
...
The cursor should be flashing in the `Directory name` field, so please click the <kbd>Browse</kbd> button then enter the `transcriptomic_applications` directory, followed by <kbd>Choose</kbd>.
...
This will start a clean R session, where have no records of our previous work, and you'll notice that in the **Files** pane we are now in the directory `~/transcriptomic_applications/practical_1`.
...
NB: If you are not in the exact directory `~/transcriptomic_applications/practical_1`, **you may be disadvantaging yourself for the remainder of the course and for any assessments that are set**. 
...
Notice that by default, you will be asked to save this in the project root directory `~/transcriptomic_applications/practical_1`.
