...
Returning to our working directory, after `/home/student/`, you should see `transcriptomics_applications/practical_2` and this is the complete path to your working directory.
...
If we wanted to look up a directory, we could try `list.files("..")` and that will step back into `/home/student/transcriptomics_applications` and you should see the directories `practical_1` and `practical_2` there.
...
It's very possible, that we provide `"data/Enrichment.csv"` as `"/home/student/transcriptomics_applications/practical_2/data/Enrichment.csv"`, and if you then sent that to an tutor who is going to mark your assignment, that folder probably won't exist on their machine.
If your tutor is `linda`, her folder may be `"/home/linda/transcriptomics_applications/practical_2/data/Enrichment.csv"`, or it may be completely different.
...
This enables portability, as by using `here::here()` on your system you'll get `"/home/student/transcriptomics_applications/practical_2/`, whilst Linda would get `"/home/linda/transcriptomics_applications/practical_2/`, or the complete path to wherever she has her `practical_2.Rproj` file.
...
```{r}
filter(Enrichment, gs_name == "GO_COPI_VESICLE_COAT")
filter(de, gs_name == "GO_COPI_VESICLE_COAT")
```
...
Find all significant gene sets containing this gene.
...
Do they belong to any significant gene sets?

(Try and perform this by searching for both of these in the same `filter()`.)
...
A *foolish thing to do* might be to use `bind_rows()` to join `Enrichment` and `de`, however this is a useful examples for demonstrating this command in action.
...
An alternative, but more useful example of this by be if we returned to some of our `group_by()` operations.
...
As a simple example, we can use a subsetting operation.
...
Here we've used `.` to indicate the object passed by `%>%` and we've then performed an operation on this.
If we apply this logic to our actual problem at hand, we can use:
...
However, it may be worth noting that `pander` will print the entire `tibble` you pass to it, so making sure you've used `filter()` or `slice()` to just return the rows you care about can be very important.